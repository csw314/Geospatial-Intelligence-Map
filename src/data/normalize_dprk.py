"""Normalization for DPRK location rows."""
...
SOURCE_FILE: Literal["dprk_data.csv"] = "dprk_data.csv"
EXPECTED_COLUMNS = (
    "Category Source",
    "Country",
    "Name",
...
OPTIONAL_COLUMNS = ("Category Source", "Notes")


def normalize_dprk_row(row: Mapping[str, Any], row_number: int) -> NormalizedLocation:
    """Normalize one DPRK CSV row into the shared location schema."""
    ...
    if coordinates is None:
        raise ValueError("invalid DPRK latitude/longitude")

    name = cleaned.get("Name") or f"Unnamed DPRK location {row_number}"
    ...
    record = LocationRecord(
        ...
        country=cleaned.get("Country") or "DPRK",
        ...
        latitude=latitude,
        longitude=longitude,
        category_source=blank_to_none(cleaned.get("Category Source")),
